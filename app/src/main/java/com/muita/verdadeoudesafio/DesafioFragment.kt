...
import com.muita.verdadeoudesafio.databinding.FragmentDesafioBinding
import kotlin.random.Random


class DesafioFragment : Fragment() {
    private lateinit var binding: FragmentDesafioBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentDesafioBinding.inflate(layoutInflater).apply {
            viewLifecycleOwner
        }
        initView()
        return binding.root
    ...
    private fun initView() {
        val nome = requireArguments().getString("UU")

        val frases : Int = (0 until (go.size)).random()
        val jhon = go[frases]

        binding.saveName.text = nome
